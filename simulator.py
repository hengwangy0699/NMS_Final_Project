import os, time, json, socket, select, random, subprocess, signal, string, hashlib, bisect, atexit, functools, sys
...
class Server:
    ...
    def server_response(self, message):
        if self.active and self.rem_port_name:
            try:
                available = select.select([], [self.sock], [], metrics["select_time"])[1]
...
class Begin:        
    ...
    def start_beg(self):
        ...
        while clock - start < metrics["total_execution_time"] and data["total_client_requests"] < metrics["max_packets"] and len(self.active_server_ids) > 0:
            ...
            ready = select.select(sockets, [], [], metrics["select_time"])[0]
            for sock in ready:
                for server in self.servers.values():
                    if server.sock == sock:
    ...
    def validation(self):
        if data["wrong_get"] or data["server_self_fail"]:
            ...
            print(len(self.server_ids), len(self.active_server_ids), data["server_fail"])
            return False
...
if __name__ == "__main__":
    ...
    metrics["number_of_clients"] = 2
    metrics["client_requests"] = 500
    metrics["servers"] = int(sys.argv[1])
    start_simulation()
